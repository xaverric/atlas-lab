...
import { UploadQueue } from '@/components/dms/upload-queue';
import type { UploadItem } from '@/components/dms/upload-queue';
...
export default function UploadPage() {
  ...
  useEffect(() => {
    api<{ data: string[] }>('/api/v1/dms/documents/tags')
      .then((res) => setAvailableTags(res.data))
      .catch(() => {});

    if (initialFolderId) {
      api<{ data: { name: string } }>(`/api/v1/dms/folders/${initialFolderId}`)
  ...
  const uploadOne = async (item: UploadItem, token: string): Promise<boolean> => {
    return new Promise((resolve) => {
      ...
      xhr.open('POST', `${baseUrl}/api/v1/dms/documents`);
  ...
  const handleSubmit = async () => {
    ...
    if (allOk) {
      ...
      setTimeout(() => router.push(`/dms${folderId ? `?folderId=${folderId}` : ''}`), 500);
    } else {
  ...
  const filteredSuggestions = availableTags.filter(
    (t) => t.toLowerCase().includes(currentTagPrefix) && !tags.split(',').map((s) => s.trim()).includes(t),
  );

  const handleFolderSelect = (id: string | null) => {
    setFolderId(id || '');
    if (id) {
      api<{ data: { name: string } }>(`/api/v1/dms/folders/${id}`)
        .then((res) => setFolderName(res.data.name))
        .catch(() => setFolderName(''));
    } else {
      setFolderName('');
