...
import { FileIcon } from '@/components/dms/file-icon';
...
export default function DocumentDetailPage() {
  ...
  useEffect(() => {
    api<{ data: Document }>(`/api/v1/dms/documents/${id}`)
      .then((res) => setDoc(res.data))
      .catch(() => toast.error('Document not found'));

    api<{ data: { url: string } }>(`/api/v1/dms/documents/${id}/preview`)
  ...
  const handleDownload = async () => {
    try {
      const res = await api<{ data: { url: string } }>(`/api/v1/dms/documents/${id}/download`);
  ...
  const handleUpdate = async (data: Record<string, unknown>) => {
    try {
      const res = await api<{ data: Document }>(`/api/v1/dms/documents/${id}`, {
        method: 'PATCH',
  ...
  const handleCreateShare = async () => {
    try {
      const res = await api<{ data: ShareToken }>('/api/v1/dms/shares', {
        method: 'POST',
  ...
  const copyShareUrl = (token: string) => {
    const url = `${window.location.origin}/api/v1/dms/shares/${token}`;
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      <button onClick={() => router.push('/dms')} className="flex items-center gap-1 text-sm text-muted-foreground active:text-foreground">
            ...
            {editingName ? (
              <div className="flex items-center gap-1 min-w-0 flex-1">
                <input
                  ...
                  className="rounded border border-input bg-background px-2 py-1 text-lg sm:text-2xl font-bold min-w-0 flex-1"
                  autoFocus
                />
                <button onClick={saveName} className="text-green-500 p-1"><Check className="h-5 w-5" /></button>
                <button onClick={() => setEditingName(false)} className="text-muted-foreground p-1"><X className="h-5 w-5" /></button>
              </div>
            ) : (
              <h1
                className="text-xl sm:text-3xl font-bold cursor-pointer active:text-primary/80 transition-colors truncate"
