...
export function DocumentTable({
  ...
  onDelete, onPreview, onRename, onMove, showPath,
}: DocumentTableProps) {
    ...
    <>
          ...
          <tbody>
            {documents.map((doc) => (
              ...
              >
                ...
                <td className="p-3">
                  <Link href={`/dms/${doc.id}`} className="flex items-center gap-2 hover:underline">
                ...
                <td className="p-3">
                  <div className="flex gap-1">
                    {canPreview(doc.mimeType) && (
                      <button onClick={() => onPreview(doc)} className="text-muted-foreground hover:text-foreground p-1" title="Preview">
                        <Eye className="h-4 w-4" />
                      </button>
                    )}
                    <button onClick={() => handleDownload(doc)} className="text-muted-foreground hover:text-foreground p-1" title="Download">
      ...
      <div className="md:hidden space-y-2">
        ...
        {documents.map((doc) => (
          ...
          >
            ...
            />
            <Link href={`/dms/${doc.id}`} className="flex flex-1 items-center gap-3 min-w-0">
              <FileIcon mimeType={doc.mimeType} className="text-muted-foreground shrink-0" />
            ...
            <button
              ...
              className="shrink-0 rounded p-1.5 text-muted-foreground hover:text-foreground active:bg-muted"
            >
      ...
      {ctxMenu && (
        <ContextMenu
          ...
          onDetails={() => router.push(`/dms/${ctxMenu.doc.id}`)}
